...
def main():
    ...
    parser.add_argument('--sync_freq', type=int, help="frequency of sync (barrier) within each epoch: sync every x batches")
    ...
    for i in r:
        ...
        if args.sync_freq:
            cmd.extend(["--param", f"train.batch_sync_frequency={args.sync_freq}"])
